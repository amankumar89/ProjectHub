...
import UserFormModal from "@/pages/Userssssss/UserFormModal";
...
const UsersPage: React.FC = () => {
  ...
  const { refetch: fetchUserById } = useGetUserById(user!.id!);
  ...
  // edit — fetch latest data first
  const handleEdit = async (user: User) => {
    setSelectedUser(user);
    const result = await fetchUserById();
    if (result.data) setSelectedUser(result?.data as User);
    setFormOpen(true);
  };
